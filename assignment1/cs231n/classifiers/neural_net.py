...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # fc1 = np.hstack((X, np.ones((N, 1)))).dot(np.vstack((W1, b1)))     #(1)
    # relu = np.maximum(fc1, 0)                                          #(2)

    scores = np.dot(hidden_layer, W2) + b2
    # fc2 = np.hstack((relu, np.ones((N, 1)))).dot(np.vstack((W2, b2)))  #(3)
    ...
    scores -= np.tile(np.reshape(np.max(scores, axis=1), (N, 1)), (1, C))
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      idx_sample = np.random.choice(num_train, size=batch_size)
